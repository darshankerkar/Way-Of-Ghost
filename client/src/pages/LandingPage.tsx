...
/* ─── Reusable ghost figure (ss3 style) ──────────────────────────────── */
export function GhostFigure({ scale = 1, className = "" }: { scale?: number; className?: string }) {
  return (
    <div
      className={`ghost-figure ${className}`}
      style={{ transform: `scale(${scale})`, transformOrigin: "center top" }}
      aria-hidden="true"
    >
      <div className="ghost-body-shape" />
      <div className="ghost-eye ghost-eye-l" />
      <div className="ghost-eye ghost-eye-r" />
      <div className="ghost-smile" />
      <div className="ghost-body-lines">
        <span /><span /><span /><span />
      </div>
      <div className="ghost-skirt" />
      <div className="ghost-reflection" />
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-2.5 flex-shrink-0">
      <svg width="52" height="30" viewBox="0 0 56 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
        <path d="M21 5.5L6.5 16" stroke="#ea4335" strokeWidth="8.5" strokeLinecap="round" />
        <path d="M21 26.5L6.5 16" stroke="#4285f4" strokeWidth="8.5" strokeLinecap="round" />
        <path d="M35 5.5L49.5 16" stroke="#34a853" strokeWidth="8.5" strokeLinecap="round" />
      ...
      <div className="flex flex-col items-start leading-[1.15] ml-0.5 pt-[1px]">
        <span className="text-[14px] font-[800] tracking-[0.01em] text-white">Google Developer Groups</span>
        <span className="text-[12px] font-[600] tracking-[0.04em] text-white/60">VIT Mumbai</span>
      </div>
    </div>
  );
}
...
/* ─── INTRO OVERLAY ──────────────────────────────────────────────────── */
function IntroOverlay({ visible, showGhost, doStrike }: {
  visible: boolean;
  showGhost: boolean;
  doStrike: boolean;
}) {
  return (
    <>
      {/* Black intro bg */}
      <div className={`contest-intro-overlay ${visible ? "contest-intro-visible" : "contest-intro-hidden"}`} aria-hidden={!visible}>
        <div className="contest-intro-stars" />
        <div className="contest-intro-core">
          {/* Ghost + tagline */}
          <div className={`contest-ghost-wrap ${showGhost ? "ghost-on" : "ghost-off"}`}>
            <GhostFigure scale={1.3} />
            <p className="ghost-tagline">Find your way out</p>
          </div>
        </div>
      </div>

      {/* Strike overlay (separate z-layer, on top) */}
      <div className={`strike-overlay ${doStrike ? "strike-active" : ""}`} aria-hidden="true">
      ...
      <div className="auth-modal-card">
        ...
        <div className="flex justify-center mb-5" style={{ height: "70px" }}>
          <div style={{ position: "relative", top: "-10px" }}>
            <GhostFigure scale={0.65} />
          </div>
        </div>
        <h2 className="text-xl font-bold text-white text-center" style={{ fontFamily: "'Orbitron',sans-serif" }}>Enter The Grid</h2>
        <p className="text-xs text-gray-400 text-center mt-1 mb-6">Welcome back, warrior.</p>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1.5">Email</label>
          ...
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1.5">Password</label>
          ...
          {error && <p className="text-xs text-ghost-red bg-ghost-red/10 rounded-lg px-3 py-2">{error}</p>}
          <button type="submit" disabled={loading} className="contest-btn-primary w-full rounded-xl py-3 font-bold text-sm disabled:opacity-60">
            {loading ? "Entering…" : "Enter Arena"}
          </button>
        </form>
        <p className="text-center text-xs text-gray-500 mt-5">
          No account?{" "}
          <button onClick={() => navigate("/register")} className="text-ghost-gold hover:underline bg-transparent border-0 cursor-pointer p-0 text-xs">
            Create Warrior ID
          </button>
        </p>
      </div>
    ...
    <div className="auth-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="auth-modal-card">
        ...
        <div className="flex justify-center mb-4" style={{ height: "60px" }}>
          <div style={{ position: "relative", top: "-10px" }}>
            <GhostFigure scale={0.6} />
          </div>
        </div>
        <h2 className="text-lg font-bold text-white text-center" style={{ fontFamily: "'Orbitron',sans-serif" }}>Create Warrior ID</h2>
        <p className="text-xs text-gray-400 text-center mt-1 mb-5">Join the ghost arena.</p>
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1">Full Name</label>
            <input className="auth-input" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1">College</label>
            <input className="auth-input" placeholder="Your college" value={college} onChange={(e) => setCollege(e.target.value)} required />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1">Email</label>
            <input className="auth-input" type="email" placeholder="your@email.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoComplete="email" />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-ghost-gold mb-1">Password</label>
          ...
          {message && <p className="text-xs text-ghost-green bg-ghost-green/10 rounded-lg px-3 py-2">{message}</p>}
          {error   && <p className="text-xs text-ghost-red bg-ghost-red/10 rounded-lg px-3 py-2">{error}</p>}
          <button type="submit" disabled={loading || !!message} className="contest-btn-primary w-full rounded-xl py-3 font-bold text-sm disabled:opacity-60">
            {loading ? "Submitting…" : "Register"}
          </button>
        </form>
        <p className="text-center text-xs text-gray-500 mt-4">
          Already a warrior?{" "}
          <button onClick={() => navigate("/login")} className="text-ghost-gold hover:underline bg-transparent border-0 cursor-pointer p-0 text-xs">
            Log in
          </button>
        </p>
      </div>
...
export function LandingPage() {
  ...
  const [introStage, setIntroStage] = useState<"ghost" | "strike" | "done">("ghost");
  ...
  useEffect(() => {
    ...
    const t1 = window.setTimeout(() => {
      if (!strikeRef.current) { strikeRef.current = true; setIntroStage("strike"); }
    }, 1100);
  ...
  return (
    <div className="contest-bg relative min-h-screen text-white">
      {/* Intro + strike */}
      <IntroOverlay visible={showIntroOverlay} showGhost={showGhost} doStrike={doStrike} />

      {/* Creepy background */}
      <div className="contest-grid-overlay absolute inset-0" aria-hidden="true" />
      <div className="creepy-fog absolute inset-0" aria-hidden="true" />
      <div className="creepy-blob-center" aria-hidden="true" />
      <CreepyParticles />
      <div className="contest-radial-glow absolute -left-24 -top-16 h-80 w-80 rounded-full" aria-hidden="true" />
      <div className="contest-radial-glow-alt contest-radial-glow absolute -right-24 bottom-8 h-96 w-96 rounded-full" aria-hidden="true" />
      ...
      <header
        className={`relative z-20 w-full px-4 pt-4 pb-2 md:px-8 transition-all duration-500 ${
          showIntroOverlay ? "-translate-y-2 opacity-0" : "translate-y-0 opacity-100"
        }`}
      >
      ...
      <div className={`mobile-menu ${menuOpen ? "menu-open" : "menu-closed"}`}>
        <button className="absolute top-5 right-5 text-gray-400 hover:text-white text-2xl bg-transparent border-0 cursor-pointer" onClick={() => setMenuOpen(false)} aria-label="Close">✕</button>
        ...
        <div className="flex flex-col gap-3 mt-5 w-full max-w-xs px-6">
          <button onClick={() => { setMenuOpen(false); navigate("/login"); }} className="contest-btn-primary rounded-xl px-6 py-3 text-sm font-semibold text-center">Enter Arena</button>
          <button onClick={() => { setMenuOpen(false); navigate("/register"); }} className="contest-btn-secondary rounded-xl px-6 py-3 text-sm font-semibold text-center">Create Warrior ID</button>
        </div>
      </div>
      ...
      <section
        className={`relative z-10 flex items-center justify-center min-h-[calc(100vh-90px)] px-4 pb-8 pt-4 transition-all duration-500 ${
          showIntroOverlay ? "translate-y-3 opacity-0" : "translate-y-0 opacity-100"
        }`}
      >
        {/* Glass card box */}
        <div className="hero-glass-card flex flex-col items-center text-center max-w-[680px] w-full">
          ...
          <p className="hero-description mb-10">
            ...
            <span className="text-[#38bdf8] font-semibold">Google Developer Groups VIT Mumbai</span>.
            Duel opponents head-to-head, survive the quiz sprint, and strategise your
            way to the throne in a three-round elimination tournament built for precision coders.
          </p>

          {/* CTA Buttons */}
          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link className="contest-btn-primary rounded-xl px-9 py-3.5 text-sm font-bold" to="/register">
              Register Now →
            </Link>
            <Link className="contest-btn-secondary rounded-xl px-9 py-3.5 text-sm font-semibold" to="/login">
      ...
      <section className="roadmap-section relative z-10 w-full px-4 pb-24 pt-10">
        ...
        <div className="roadmap-canvas mx-auto">
          ...
          >
            ...
            <ellipse cx="300" cy="190" rx="80" ry="18" fill="rgba(30,180,100,0.09)" />
            <ellipse cx="560" cy="128" rx="95" ry="22" fill="rgba(80,40,140,0.10)" />
            <ellipse cx="820" cy="88"  rx="70" ry="16" fill="rgba(30,180,100,0.08)" />

            {/* ── NODE 1 — Round 1 ──────────────────────── */}
            {/* connector stem */}
            <line x1="235" y1="175" x2="235" y2="100" stroke="rgba(231,76,60,0.5)" strokeWidth="1.5" strokeDasharray="4 4" className="node-stem" />
            {/* outer ring */}
            <circle cx="235" cy="175" r="18" fill="rgba(231,76,60,0.15)" stroke="rgba(231,76,60,0.7)" strokeWidth="1.5" filter="url(#node-glow)" className="road-node" />
            {/* inner dot */}
            <circle cx="235" cy="175" r="7" fill="#e74c3c" filter="url(#node-glow)" />
            {/* label box */}
            ...
            {/* ── NODE 2 — Round 2 ──────────────────────── */}
            <line x1="510" y1="115" x2="510" y2="38" stroke="rgba(56,189,248,0.5)" strokeWidth="1.5" strokeDasharray="4 4" className="node-stem" style={{ animationDelay: "0.3s" }} />
            <circle cx="510" cy="115" r="18" fill="rgba(56,189,248,0.15)" stroke="rgba(56,189,248,0.7)" strokeWidth="1.5" filter="url(#node-glow)" className="road-node" style={{ animationDelay: "0.4s" }} />
            <circle cx="510" cy="115" r="7" fill="#38bdf8" filter="url(#node-glow)" />
            ...
            {/* ── NODE 3 — Round 3 ──────────────────────── */}
            <line x1="790" y1="80" x2="790" y2="260" stroke="rgba(251,191,36,0.5)" strokeWidth="1.5" strokeDasharray="4 4" className="node-stem" style={{ animationDelay: "0.6s" }} />
            <circle cx="790" cy="80" r="18" fill="rgba(251,191,36,0.15)" stroke="rgba(251,191,36,0.7)" strokeWidth="1.5" filter="url(#node-glow)" className="road-node" style={{ animationDelay: "0.7s" }} />
            <circle cx="790" cy="80" r="7" fill="#fbbf24" filter="url(#node-glow)" />
            ...
            {/* Finish flag glow */}
            <circle cx="975" cy="55" r="10" fill="rgba(251,191,36,0.2)" stroke="rgba(251,191,36,0.6)" strokeWidth="1" filter="url(#node-glow)" className="road-node finish-node" />
            <text x="975" y="59" textAnchor="middle" fill="#fbbf24" fontSize="10" fontFamily="sans-serif">🏁</text>
          </svg>
        </div>
      </section>

      <div className="contest-bottom-line pointer-events-none absolute bottom-8 left-1/2 h-px w-[85%] -translate-x-1/2" aria-hidden="true" />
      <div className="contest-scan-line pointer-events-none absolute inset-x-0 top-0 h-24" aria-hidden="true" />
