...
export function Round2Page() {
  ...
  const { fullscreenLockActive, violationLocked, reEnterFullscreen } =
    useExamMode("Round 2", 2, !isWinner);

  const fetchMatchup = useCallback(async () => {
    if (!user) return;
    try {
      const { data } = await http.get<Matchup[]>("/round/2/matchups");
      ...
      if (myMatch) {
        ...
        const { data: problems } =
          await http.get<Problem[]>("/problem?round=2");
        const p = problems.find((pr) => pr.id === myMatch.problem.id);
        if (p) {
          setProblem(p);
          if (isNewMatchup) {
            // Check local storage for existing code first
  ...
  useEffect(() => {
    if (matchup && Object.keys(code).length > 0) {
      localStorage.setItem(`wog_r2_code_${matchup.id}`, JSON.stringify(code));
    }
  }, [code, matchup]);
  ...
  useEffect(() => {
    ...
    const handleRoundStarted = (data: { roundNumber: number }) => {
      if (data.roundNumber === 2) fetchMatchup();
    };
  ...
  useEffect(() => {
    ...
    const socket = getSocket();
    socket?.emit("exam:exclude", { roundNumber: 2 });
  }, [matchResult, user]);
