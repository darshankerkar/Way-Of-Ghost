...
import { GhostFigure, GdgLogo } from "../pages/LandingPage";
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden sm:block" style={{ width: "24px", height: "28px", overflow: "hidden", position: "relative" }}>
              <div style={{ position: "absolute", top: "-4px", left: "-8px" }}>
                <GhostFigure scale={0.27} />
              </div>
            </div>
            <span className="hidden sm:block text-xs text-gray-400 max-w-[110px] truncate">{user.name}</span>
            {user.role === "ADMIN" && (
      ...
      <div className={`mobile-menu ${menuOpen ? "menu-open" : "menu-closed"}`}>
        ...
        {/* Ghost in mobile menu */}
        <div className="mb-2 ghost-on" style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <GhostFigure scale={0.75} />
        </div>
        {navLinks.map((link) => (
